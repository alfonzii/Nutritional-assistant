...
Simple android application for ___nutrition control___ and ___weight goal___ achievement with ___adaptive meal plan generation___ created in _Java_ with _Android SDK_.

___Biggest asset___ of this application in comparison to similar nutrition control/manager apps on market is that users ___don't have to strictly obey given meal plan___. Instead, they can ___deviate___ from it if needed and still be able to ___meet___ their nutrition ___limits___ and goals by ___using___ application's ___adaptive meal plan re-generation___.
